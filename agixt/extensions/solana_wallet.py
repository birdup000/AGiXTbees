...
def create_solana_wallet():
    """Create a new Solana wallet and return private key, passphrase, and address"""
    # Generate new random keypair
    keypair = Keypair()
    # Get private key bytes and encode as hex string
    private_key = bytes(keypair)[:32].hex()
    # We don't use a passphrase in this implementation
    passphrase = ""
    return private_key, passphrase, str(keypair.pubkey())
...
class solana_wallet(Extensions):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Retrieve wallet details directly from agent config
        WALLET_PRIVATE_KEY = kwargs.get("SOLANA_WALLET_API_KEY", None)
        WALLET_ADDRESS = kwargs.get("SOLANA_WALLET_ADDRESS", None)
        ...
        if WALLET_PRIVATE_KEY:
            ...
            # Verify the stored address matches the derived one, or just use the stored one
            derived_address = str(self.wallet_keypair.pubkey())
            if WALLET_ADDRESS and WALLET_ADDRESS == derived_address:
                self.wallet_address = WALLET_ADDRESS
            elif WALLET_ADDRESS:
                # Log a warning if they don't match, but prefer the stored address
                logging.warning(f"Stored SOLANA_WALLET_ADDRESS ({WALLET_ADDRESS}) does not match address derived from SOLANA_WALLET_API_KEY ({derived_address}). Using stored address.")
                self.wallet_address = WALLET_ADDRESS
            else:
                # If address wasn't stored for some reason, use the derived one
                logging.warning(f"SOLANA_WALLET_ADDRESS not found in settings, using address derived from key: {derived_address}")
